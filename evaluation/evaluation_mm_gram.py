"""VERBATIM copy of GRAM's evaluation/evaluation_mm.py (github.com/ispamm/GRAM, main).

Do not edit. This exists so our numbers can be compared against GRAM's published table under
GRAM's OWN scoring, rather than ours. The one substantive difference in our version is that the
task string selects the volume's arity: `ret%tv` builds a 2-modal volume, `ret%tva` a 3-modal one,
and so on. Upstream instead builds the volume over EVERY modality the loader supplied and ignores
the task string, so its tv / tva / tvas rows are the same volume with different ITM conditioning.

Ours is arguably the more faithful reading of the paper's text ("with two modalities the volume
computation degenerates to the area of the triangle"), but it means our lower-arity rows measure
something the published table does not. Selected with GRAM_UPSTREAM_EVAL=1; see run_eval.py.
"""
import os
import json
import torch
