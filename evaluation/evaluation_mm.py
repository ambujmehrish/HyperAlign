...
def evaluate_ret(model, tasks, val_loader, global_step):
    ...
    if len(feat_v) > 0:        # audio-only (AudioCaps T-A): no video -> skip Gramian-volume/cosine-video block
        ...
        # ---- The TASK STRING selects the volume's modalities (GRAM paper Table 1/4 protocol).
        # Before: the volume always used every modality the batch happened to carry, so
        # ret%tv / ret%tva / ret%tvas produced the SAME volume (measured: 39.6 for all three on
        # MSR-VTT) and only the ITM condition differed -- i.e. the "T-V" row was not text+video at
        # all. The paper is explicit that with two modalities "the volume computation degenerates
        # to the area of the triangle", so each row there is a different-arity volume.
        # volume_computation(language, *inputs) is GRAM's own arity-generic form (verified
        # numerically identical to volume_computation3/4/5, and it is the only one that can do the
        # 2-modality case); it ships in utils/volume.py but is never called anywhere in the repo.
        # A modality the task asks for but that is absent is a HARD ERROR: dropping it silently
        # would compute a lower-arity volume and still report it under the requested task name
        # (e.g. tvasd with no depth would quietly become tvas), which is exactly the failure above.
        # Everything below this point reports ONE result set, and the ITM block further down keys
        # store_dict by `task` -- the last subtask, leaked from the loop above. So with >1 subtask
        # the volume would come from subtasks[0] while the ITM re-rank used the last one: a silent
        # mismatch. Every config we run carries exactly one val subtask; fail loudly if that breaks
        # (with --first_eval true this fires in the first minutes, not after hours of training).
        assert len(subtasks) == 1, (
            f"val task has {len(subtasks)} subtasks {subtasks}: the volume would be built from "
            f"'{subtasks[0]}' but the ITM re-rank keyed on '{subtasks[-1]}'. Give the val config "
            f"one subtask (e.g. 'ret%tvas'), or run one eval per subtask.")
        _task = subtasks[0]
        _mods = _task[1:]                   # 'v' | 'va' | 'vas' | 'vasd' | 'a' ...
        _avail = {'v': feat_v, 'a': feat_a, 's': feat_s, 'd': feat_d}
        _feats = []
        for _m in 'vasd':
            if _m not in _mods:
                continue
            _f = _avail[_m]
            if not torch.is_tensor(_f) or _f.shape[0] == 0:
                raise RuntimeError(
                    f"task {_task} requires modality '{_m}' for the volume, but it is absent. "
    ...
    ## compute itc_score
    for task in subtasks:
        if  task == "tvas" or task == "tva" or task == "tvasd":
            continue
    ...
    for task in subtasks:
        if  task == "tvas" or task == "tva" or task == "tvasd":
            continue
        if task!="tvas" and task!="tva" and isinstance(store_dict[f'condition_feats_{task}'], list):
            store_dict[f'condition_feats_{task}'] = torch.cat(store_dict[f'condition_feats_{task}'],dim=0)
        itm_rerank_num = model.config.itm_rerank_num
