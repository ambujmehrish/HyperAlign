...
class AnnoIndexedDataset(Dataset):
    def __init__(self, d_cfg, args):
        ...
        # Extension-agnostic set of clips whose VIDEO file exists (any container: .mp4/.mkv/.webm).
        # A clip that passes the .wav filter but has no loadable video makes read() return None,
        # and __getitem__ then resamples a RANDOM other clip -> that clip appears twice and this
        # one vanishes, corrupting the eval gallery. Requiring the video here removes that
        # substitution at the source. The collate cannot drop a per-item None (it would torch.stack
        # None with tensors and crash), so filtering up front is the clean approach.
        _vid = set()
        if 'vision' in d_cfg:
            try:
                for _f in os.listdir(d_cfg['vision']):
                    _vid.add(os.path.splitext(_f)[0])
            except OSError:
                pass
        ...
        for key in self.annos:
            ...
            # video-retrieval sets: require ONLY the anchor modality (video); a missing .wav is
            # zero-filled and masked out of that clip's volume (per-clip arity). No modality filter.
            if self.name == "didemo_ret":
                if key['video_id'] in _vid:
                    self.annos_new.append(key)
            ...
            if self.name == "activitynet_ret":
                if key['video_id'] in _vid and "desc" in key:
                    self.annos_new.append(key)
            if self.name == "audiocaps_ret":     # audio-captioning: audio is the anchor; video optional -> masked
                path = os.path.join(d_cfg['audio'],f"{key['video_id']}.wav")
                if os.path.exists(path) and os.path.getsize(path) > 2000:     # drop only 0-dur/stub audio
                    self.annos_new.append(key)
            ...
            if self.name == "vast27m":     # our VAST-150k: keep every clip that has VIDEO; audio is
                clip_ok = os.path.exists(os.path.join(d_cfg['vision'], f"{key['clip_id']}.mp4"))
                if clip_ok:                 # optional -> a missing .wav is zero-filled and masked out of
                    self.annos_new.append(key)  # that clip's volume (per-clip arity). No modality filter.

            if self.name == "vgg_ret":     # VGGSound-5K T-AV classification: keep clips with video+audio
                vid_ok = os.path.exists(os.path.join(d_cfg['vision'], f"{key['video_id']}.mp4"))
                wav_ok = os.path.exists(os.path.join(d_cfg['audio'],  f"{key['video_id']}.wav"))
                if vid_ok and wav_ok:
                    self.annos_new.append(key)



        _orig = len(self.annos)
        self.annos = self.annos_new
        _kept = len(self.annos)
        LOGGER.info(f"[DATA-FILTER] {self.name}: annotation {_orig} -> kept {_kept} "
                    f"(dropped {_orig - _kept} clips missing the ANCHOR modality only). "
                    f"Secondary-modality gaps are kept: zero-filled and masked per-clip (no modality filter).")
    ...
    def __getitem__(self, i):
        ...
        vision_pixels = None       # default so audio-only datasets (no vision_mapper) don't hit
        depth_pixels = None        # UnboundLocalError on the return tuple below
