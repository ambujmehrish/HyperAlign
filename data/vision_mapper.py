...
import glob
import cv2
cv2.setNumThreads(0)   # cv2 internal threads deadlock inside forked DataLoader workers -> training hangs
import sys
...
class VisionMapper(object):
    def __init__(self, d_cfg, args):
        ...
        # Extension map, built ONCE at init (O(N)), then O(1) per clip.
        # GRAM's check_extension() globbed the whole folder PER CLIP -> O(N^2), which stalled
        # data loading, so it was removed; but hardcoding ".mp4" silently loses every clip stored
        # under another container. ActivityNet keeps 318 of its videos as .mkv: those failed to
        # load, hit IndexAnno's random resample, and put duplicates in the eval gallery. This map
        # restores GRAM's extension-agnostic lookup without its cost.
        self.ext_map = {}
        try:
            for f in os.listdir(self.vision):
                stem = os.path.splitext(f)[0]
                if stem not in self.ext_map or f.endswith('.mp4'):   # prefer .mp4 on collision
                    self.ext_map[stem] = f
        except OSError:
            pass          # vision dir absent (audio-only cfg) -> read() falls back to .mp4
        # DEPTH is CONFIG-DRIVEN: 5-model config sets d_cfg.depth (dir of {id}_depth.npy);
        # 4-model config has no 'depth' key -> self.depth_dir=None -> no depth ever (stays 4-modal
        # even if depth files exist on disk). This is what separates 4-model from 5-model.
        self.depth_dir = getattr(d_cfg, 'depth', None)
        ### for feat extraction
            ...
            if self.vision_format == 'video_feats':
                ...
                try:
                    if self.vision_format == 'video_rawvideo':
                        ...
                        # O(1) lookup in the init-time extension map (see __init__): finds the clip
                        # whatever container it is stored in (.mp4/.mkv/.webm/...), like GRAM's
                        # check_extension did, without its per-clip glob. Falls back to .mp4 so a
                        # clip genuinely absent from disk still yields a missing path (-> resample),
                        # exactly as before.
                        _fname = self.ext_map.get(str(id_))
                        video_path = os.path.join(self.vision, _fname) if _fname else video_path + ".mp4"
                        # video_path = check_extension(id_,self.vision)  # REMOVED: this globbed all 136k
                        # files in the folder PER clip (O(N^2)) -> data loading stalled, no GPU training.
                        ...
                        # DEPTH is CONFIG-DRIVEN: only the 5-model config sets d_cfg.depth.
                        # 4-model config -> self.depth_dir is None -> depth_pixels stays None
                        # -> 4-modal (even if depth .npy files exist on disk). This is the switch.
                        if self.depth_dir:
                            depth_path = os.path.join(self.depth_dir, f"{id_}_depth.npy")
                            if not os.path.exists(depth_path):
                                return None, None   # missing depth -> resample (training)
