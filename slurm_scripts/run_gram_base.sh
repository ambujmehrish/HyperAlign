...
#SBATCH --mem=240G
#SBATCH --job-name=gram_base
#SBATCH -o /leonardo/home/userexternal/amehrish/HyperAlign/slurm_scripts/logs/gram_base_%j.out
#SBATCH -e /leonardo/home/userexternal/amehrish/HyperAlign/slurm_scripts/logs/gram_base_%j.out
...
# Writes to workdir_gram_base/4model. Resume only triggers off that directory, so a first run
# always starts from the VAST pretrained weights.
echo "START=$(date +%T) [GRAM-BASE ablation (stage A, hypergraph OFF) -> ./workdir_gram_base/4model]"
...
srun python3 -m torch.distributed.launch --nnodes 1 --node_rank 0 --nproc_per_node 4 --master_port 9893 \
  ./run.py --config ./config/gram/pretrain_cfg/gram_base.json \
  --output_dir "$W/workdir_gram_base/4model" --checkpointing true $RESUME 2>&1
echo "DONE $(date +%T)"
